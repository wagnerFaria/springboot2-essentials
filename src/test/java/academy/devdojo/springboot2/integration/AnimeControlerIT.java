...
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_EACH_TEST_METHOD) // usado apenas em testes de integração. Para zerar tudo antes de cada teste
class AnimeControlerIT {
    ...
    //caso queira pegar a porta durante a execução dos testes
//    @LocalServerPort
//    private int port;
    ...
    void findByName_ReturnsAnEmptyListOfAnimes_WhenAnimeIsNotFound() {
        String url = String.format("/animes/find?name=%s", "One anime");
        List<Anime> animes = testRestTemplate
                .exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<Anime>>() {
                }).getBody();
        Assertions.assertThat(animes)
